...
- `npx playwright test tests/e2e/shell-foundation-smoke.spec.js` from `frontend/`: passing, includes shell status-bar progress, source-intake tray routing, and shell-off rollback coverage.
- `npx playwright test tests/e2e/selection-shell-regression.spec.js tests/e2e/review-tray-regression.spec.js tests/e2e/shell-foundation-smoke.spec.js --workers=1` from `frontend/`: passing, 25 passed and 1 intentional `fixme` skip after the shell-off rollback smoke slice.
...
| Shell-off compatibility remains covered | Feature flag rollback path | Default-on rollout cannot be safely disabled or corrupts existing workspace data | Run shell-off smoke/manual pass and confirm legacy FloatingDock surfaces still open, edit, save, and reopen existing workspace data | Shell-off smoke now covers legacy workspace and metadata FloatingDock mounts; deeper edit/save/reopen rollback pass still open | Yes |
...
landed. Projection helper extraction and first projection-level tests have
landed. Manual visual QA remains.
...
- Keep branch scope, selected node state, and relationship-label state visually
  distinct.
- Keep the projection-level tests for semantic relationship edge exclusion from
  strong branch emphasis green before broad lens expansion.

Avoid:

- New popups.
- Large `projectCanvasGraph` behavior changes without focused projection tests.
