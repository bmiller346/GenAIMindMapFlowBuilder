...
const flowId = flowStore((s) => s.flow_id);
    ...
    const addDataSource = (e) => {
        ...
        const activityId = addActivity({
            title: 'Adding image source',
            detail: file?.name,
            context: 'Uploading image and deriving workspace structure.'
        });
        const data = {
            file: file,
            operationId
        ...
        pushNode(LoadingModal, {
            ...sourceUploadLoading('image', file?.name),
            operationId,
            onCancel: () => {
                controller.abort();
                updateActivity(activityId, {
                    status: 'canceled',
                    context: 'Image upload request was canceled.'
                });
                popNode();
            }
        });
        const undoSourceAdd = createSourceUndoHandler({
            ...
            setViewPort,
            setViewport,
            context: 'Image source add was undone.'
        });
        ...
        axios
            ...
            .then((res) => {
                updateActivity(activityId, {
                    status: 'completed',
                    context: 'Image source was added to the workspace.',
                    undo: undoSourceAdd
                });
                setupNodes(res.data);
            })
